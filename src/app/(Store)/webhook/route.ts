...
export async function POST(req: NextRequest) {
    ...
    if (event.type === "checkout.session.completed") {
        ...
        try {
            ...
            console.log("Order in Sanity", order);
        }
        catch (error) {
...
async function createOrderInSanity(session: Stripe.Checkout.Session) {
    ...
    const order = await backendClient.create({
        _type: "order",
        orderNumber,
        stripeCheckoutSessionId: id,  // corrected typo
        stripePaymentIntentId: payment_intent,
        customerName,
        stripeCustomerId: customer,
        clerkUserId: clerkUserId,   // corrected typo
        email: customerEmail,
        currency,
        amountDiscounted: total_details?.amount_discount  // corrected typo
