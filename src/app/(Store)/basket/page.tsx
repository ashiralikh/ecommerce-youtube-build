...
const hendeleCheckout = async () => {
    if(!isSignedIn) return ;
    setIsLoading(true);   

    try{
           const metadata :Metadata = {
               orderNumber: crypto.randomUUID(), // example: 'abc123_def456_ghi789'
               customerName:user?.fullName ?? 'unknown',
               customerEmail:user?.emailAddresses[0].emailAddress ?? 'unknown',
               clerkUserId:user!.id,             
           };

           const chekoutUrl = await createCheckoutSession(groupItems ,metadata);
...
//   console.log('Baket Content', groupItems);
  return (
     ...
     <div className="flex flex-col sm:flex-row md:flex-row gap-8">
        <div className='flex-grow'>
            {groupItems?.map((item) => (
                ...
                className="mb-4 p-4 border justify-between items-center border-gray-200 rounded-lg shadow-md">
        ...
        <div className="w-full lg:w-80 lg:sticky lg:top-4 h-fit bg-white p-6
        border border-gray-200 rounded order-first lg:order-last fixed bottom-0 left-0 lg:left-auto ">
            ...
            <div className='mt-4 space-y-2'>
                ...
                <p className="flex justify-between border-t ">
                    <span className='font-bold justify-between flex text-2xl pt-2'>Total: 
                            
                        </span>
                        <span className='font-bold justify-between flex text-2xl  pt-2'>
                        £{groupItems.reduce((total, item) => total + (item.product.price ?? 0) * item.quantity, 0).toFixed(2)}</span>
                </p>
                <p className="flex justify-between border-t ">  
                    <span className='font-bold justify-between flex text-2xl pt-2'>
                    Subtotal:
